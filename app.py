...
def index():
    ...
    if news_id != None and news_unique_id != None and resource_folder != None and file_name != None:
        ...
        response_second = requests.get(url_second, headers=headers_second, cookies=cookies)
...
def create_short_url():
    ...
    existing_short_key = next((k for k, v in short_urls.items() if v == decoded_url), None)
...
def redirect_to_original_url():
    ...
    if short_key in short_urls:
        return redirect(short_urls[short_key])
    else:
